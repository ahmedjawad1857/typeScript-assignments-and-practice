...
// // Ans.
var arr: number[] = [2, 6, 7, 53, 2, 4, 6, 7, 8];
for (const i of arr) {
  console.log(i);
}
...
var calculateAverage = (grade: number[], total: number) => {
...
// // For Example:
let myTuple: [boolean, number, string, number[]];
...
class student extends Person {
  Class: number;
  constructor(age: number, name: string, Class: number) {
    super(age, name);
    this.Class = Class;
  }
}
...
let indexP: any = prompt(`Enter a number`);
let index: number = 1;
// while (index <= 10) {
//   index++;
// }
